...
- **Управление и чат на главной**: слева метрики, терминал, файлы, питание; справа — Cursor Agent
...
Всё на **`/`** — без вкладок и отдельных разделов. `/settings`, `/chat`, `/remote` перенаправляют на главную.
...
## Управление и чат (главная `/`)

| Слева | Справа |
|-------|--------|
| Терминал, файлы, питание, скриншот | Чат с **Cursor Agent** |

Пароль не используется — **не открывайте порт в интернет**. Скриншот: `sudo apt install grim scrot` при необходимости.

С телефона можно писать задачи в чат справа — на ноуте их выполняет **Cursor Agent** (CLI).
