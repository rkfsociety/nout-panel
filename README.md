...
- **Страницы**: `/` — метрики в шапке + управление, `/chat` — агент, `/settings` — конфиг и логи
...
| URL | Содержимое |
|-----|------------|
| `/` | Метрики в шапке; терминал, файлы, питание, скриншот |
| `/remote` | Редирект на `/` |
| `/chat` | Чат с Cursor Agent |
| `/settings` | Конфиг, логи, перезапуск панели |

Сверху на всех страницах — общая шапка с метриками; снизу — футер. Слева — кнопки-блоки между разделами.
...
- **Датчики**: нет температуры на ПК — в UI просто не показывается, панель не падает
